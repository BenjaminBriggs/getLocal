...
module Getlocal
    ...
    class CLI < Thor
        ...
        method_option :custom, :type => :boolean, :aliases => "-c"
        ...
        def fetch(project)
            ...
            if options[:custom]
                # Fetch a single file if a filename has been provided
                puts "Provide a Filename (.strings)..."
                filename = STDIN.gets.chomp
                if !filename.end_with?(".strings")
                    filename = filename + ".strings"
                end
                    
                fetchFile(project, filename, supportedLanguages, auth, options[:timeout], sleepTime)
                else
                ...
                Dir.glob("Base.lproj/*.strings") do |stringFilePath|
                    ...
                    fetchFile(project, stringFilePath, supportedLanguages, auth, options[:timeout], sleepTime)
                end
            end
        end
        ...
        def fetchFile(project, stringFilePath, supportedLanguages, auth, timeout, sleepTime)
            ...
            supportedLanguages.each do |lang|
                ...
                begin
                    response = HTTParty.get("https://api.getlocalization.com/#{project}/api/translations/file/#{fileName}/#{lang}/", :basic_auth => auth, :timeout => timeout)
                    rescue
